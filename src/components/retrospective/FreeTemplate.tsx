  ...
  useEffect(() => {
    ...
    async function createEditor() {
      ...
      try {
        ...
        const editor = new EditorJSModule({
          ...
          onChange: async () => {
            ...
            if (holderRef.current) {
              const editorElements = holderRef.current.querySelectorAll('.ce-block__content, .ce-paragraph, .ce-header, [contenteditable]');
        ...
        // EditorJS 텍스트 왼쪽 정렬 강제 적용
        if (holderRef.current) {
          // 에디터 내부 요소에 직접 스타일 적용
          setTimeout(() => {
            const editorElements = holderRef.current?.querySelectorAll('.ce-block__content, .ce-paragraph, .ce-header, [contenteditable]');
            editorElements?.forEach((el) => {
              (el as HTMLElement).style.textAlign = 'left';
            ...
            // placeholder도 왼쪽 정렬
            const placeholders = holderRef.current?.querySelectorAll('[data-placeholder]');
            placeholders?.forEach((el) => {
              const htmlEl = el as HTMLElement;
              htmlEl.style.textAlign = 'left';
              const style = window.getComputedStyle(htmlEl, '::before');
              if (htmlEl) {
                htmlEl.setAttribute('style', 'text-align: left !important;');
              }
            });
          }, 100);
        }
